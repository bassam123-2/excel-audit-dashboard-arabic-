...
| Command | Use case |
|---------|----------|
| `python manage.py runserver 0.0.0.0:8765` | Standard Django dev server |
...
If parsing fails after your change, check the Arabic column names and that you did not break normalization logic used by `apps/dashboard/services/legacy_bridge.py`.
...
| Filters, APIs, Excel logic | `apps/dashboard/`, `apps/legal/`, `apps/uploads/` |
| Brand logos by subsidiary | `assets/logos/`, `assets/logos/mapping.json`, `apps/branding/` |
...
2. Run migrations, then test with a valid Excel file (upload, filters, legal-text modal, export if touched).
...
- Steps to reproduce
- Whether it happens in `python manage.py runserver` only, or also in the built `.exe`
